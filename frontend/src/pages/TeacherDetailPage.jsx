...
const DifficultyDisplay = ({ easy, medium, hard }) => {
  ...
  if (total === 0) return <Typography color="text.secondary">데이터 없음</Typography>;
...
const MentionList = ({ mentions, mentionsLoading }) => {
  if (mentionsLoading) return <CircularProgress />;
  ...
  return (
    <List>
      {mentions.map((mention, index) => (
        <Box key={mention.id || index}>
          <ListItem alignItems="flex-start">
            <ListItemText
              primary={
                <Box display="flex" alignItems="center" gap={1}>
                  <Chip label={mention.mentionType} size="small" variant="outlined" />
                  <Chip
...
const ReportHistory = ({ reports, reportsLoading }) => {
  if (reportsLoading) return <CircularProgress />;
...
function TeacherDetailPage() {
  ...
  const fetchTeacherData = async (teacherId) => {
    ...
    try {
      const [teacherRes, mentionsRes, historyRes] = await Promise.all([
        teacherService.getById(teacherId).catch(() => ({ data: null })),
        teacherService.getMentions(teacherId, { limit: 10 }).catch(() => ({ data: [] })),
        teacherService.getReports(teacherId, { days: 7 }).catch(() => ({ data: [] })),
      ]);
      ...
      if (historyRes.data?.length > 0) {
        setTodayReport(historyRes.data[0]);
      }
    } catch (err) {
    ...
    <Container maxWidth="lg" sx={{ py: 3 }}>
        ...
        <Grid container spacing={3} alignItems="center">
          ...
          <Grid item xs>
            ...
            <Box display="flex" gap={2} mt={1}>
              <Chip
                icon={<SchoolIcon />}
                label={teacher.academy_name || '학원 정보 없음'}
                variant="outlined"
              />
              <Chip
                icon={<MenuBookIcon />}
                label={teacher.subject_name || '과목 정보 없음'}
                variant="outlined"
              />
            </Box>
      ...
      <Grid container spacing={3}>
        <Grid item xs={12} md={4}>
          <Paper sx={{ p: 2, height: '100%' }}>
            ...
            {todayReport ? (
              <Box>
                <Box mb={2}>
                  <Typography variant="body2" color="text.secondary">총 언급</Typography>
                ...
                <Box mb={2}>
                  <Typography variant="body2" color="text.secondary">감성 분석</Typography>
                  <SentimentBar
                ...
                <Box mb={2}>
                  <Typography variant="body2" color="text.secondary">난이도 평가</Typography>
                  <DifficultyDisplay
                ...
                <Box>
                  <Typography variant="body2" color="text.secondary">추천 언급</Typography>
                  <Box display="flex" alignItems="center" gap={1}>
                    <ThumbUpIcon color="primary" />
                    <Typography variant="h5">{todayReport.recommendationCount || 0}</Typography>
                  </Box>
                </Box>
              </Box>
            ) : (
              <Typography color="text.secondary">오늘 데이터가 없습니다.</Typography>
            )}
          </Paper>
        </Grid>
        ...
        <Grid item xs={12} md={8}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="h6" gutterBottom>최근 멘션</Typography>
            <Divider sx={{ mb: 2 }} />
            <MentionList mentions={mentions} mentionsLoading={false} />
          </Paper>
        </Grid>

        <Grid item xs={12}>
          <Paper sx={{ p: 2 }}>
            <Typography variant="h6" gutterBottom>최근 7일 리포트</Typography>
            <Divider sx={{ mb: 2 }} />
            <ReportHistory reports={reportHistory} reportsLoading={false} />
          </Paper>
        </Grid>
        ...
        {todayReport?.summary && (
          <Grid item xs={12}>
            <Paper sx={{ p: 2, bgcolor: 'primary.light' }}>
              ...
              <Typography color="primary.contrastText">{todayReport.summary}</Typography>
            </Paper>
          </Grid>
        )}
      </Grid>
