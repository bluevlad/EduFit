"""
EduFit AI Crawler SQLAlchemy Models
강사 평판 분석 시스템
"""
...
class Academy(Base):
    ...
    name_en = Column(String(100))  # EduFit 확장: 영문명
    code = Column(String(50), unique=True, nullable=False)
    website = Column(String(200))
    logo_url = Column(String(300))
    keywords = Column(ARRAY(Text))  # EduFit 확장: 검색 키워드
...
class CollectionSource(Base):
    ...
    is_active = Column(Boolean, default=True)
    config = Column(JSONB)
    last_crawled_at = Column(DateTime)  # EduFit 확장: 마지막 크롤링 시각
    created_at = Column(DateTime, default=datetime.utcnow)
...
class DailyReport(Base):
    ...
    # AI 요약
    summary = Column(Text)
    top_keywords = Column(ARRAY(Text))
...
class WeeklyReport(Base):
    ...
    mention_change_rate = Column(Float)
...
class SystemConfig(Base):
    ...
    environment = Column(String(20), default='all')
    created_at = Column(DateTime, default=datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
